...
const Navbar: React.FC<{ onChangeCity: () => void }> = ({ onChangeCity }) => {
  const { t } = useLang();
  const tn = t.temuco.nav;
  const scrolled = useScrolled();
  const [open, setOpen] = useState(false);

  const links = [
    { label: tn.rooms, href: '#salas' },
    { label: tn.howItWorks, href: '#como-funciona' },
    { label: tn.pricing, href: '#precios' },
        ...
        <div className="max-w-4xl mx-auto w-full">
          <div className={`transition-all duration-1000 ease-out ${vis ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`} style={{ transitionDelay: '200ms' }}>
            <div className="flex items-center gap-3 mb-5">
              ...
              <span className="text-[10px] font-black tracking-[0.4em] uppercase" style={{ color: N }}>{h.label}</span>
            </div>
            <h1 className="text-5xl md:text-7xl lg:text-8xl font-black text-white leading-none mb-4 tracking-tight" style={{ fontFamily: "'Cinzel', serif" }}>
              {h.title1}
              <br />
              <span style={{ color: N, textShadow: `0 0 30px ${N_GLOW}, 0 0 60px rgba(0,255,136,0.15)` }}>{h.title2}</span>
            </h1>
          </div>
        ...
        <div className="grid md:grid-cols-2 gap-16 items-center">
          <div className={`transition-all duration-1000 ease-out ${vis ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10'}`}>
            <div className="flex items-center gap-3 mb-5">
              ...
              <span className="text-[10px] font-black tracking-[0.4em] uppercase" style={{ color: N }}>{a.label}</span>
            </div>
            <h2 className="text-4xl md:text-5xl font-black text-white mb-6 leading-tight" style={{ fontFamily: "'Cinzel', serif" }}>
              {a.title1} <span style={{ color: N }}>{a.title2}</span>
            </h2>
            <p className="text-white/55 text-base leading-relaxed mb-4">{a.desc1}</p>
            ...
            <div className="space-y-3 mb-9">
              {[
                ...
                <div key={item.title} className="flex items-start gap-3">
                  <CheckCircle size={15} style={{ color: N, flexShrink: 0, marginTop: 2 }} />
                  <span className="text-white/60 text-sm"><span className="text-white/80 font-semibold">{item.title}</span> — {item.desc}</span>
                </div>
              ))}
            </div>
            <NeonBtn href="#salas">
              {t.temuco.rooms.label} <ArrowRight size={15} />
            </NeonBtn>
          </div>
                ...
                {!room.available && (
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className="text-center">
                      ...
                      <span className="text-white/30 text-xs font-bold tracking-widest uppercase">{r.comingSoon}</span>
                    </div>
                  </div>
                )}
...
const HowItWorks: React.FC = () => {
  ...
  return (
    <section ref={ref} id="como-funciona" className="py-28" style={{ background: '#0a0a0a' }}>
      <div className="max-w-6xl mx-auto px-6">
        <div className={`transition-all duration-1000 ease-out ${vis ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <SecHeader label={hw.label} title={<>Cómo <span style={{ color: N }}>{hw.title.split(' ').slice(1).join(' ') || 'Funciona'}</span></>} />
        </div>
          ...
          {hw.steps.map(({ title, desc }, i) => (
            <div key={title}
              ...
              <div className="w-12 h-12 rounded-xl flex items-center justify-center mb-6 transition-all duration-300 group-hover:scale-110"
                ...
                {[<Users size={22} />, <MapPin size={22} />, <Zap size={22} />, <Clock size={22} />][i]}
              </div>
...
const WhyUs: React.FC = () => {
  const { t } = useLang();
  const w = t.temuco.whyUs;
  const [ref, vis] = useInView();
  const icons = [<Star size={24} />, <Lock size={24} />, <Users size={24} />, <Clock size={24} />];
...
const Testimonials: React.FC = () => {
  ...
  const tl = t.temuco.testimonials;
  const [ref, vis] = useInView();
  const [current, setCurrent] = useState(0);

  const testimonials = lang === 'en' ? [
    { text: 'Incredible experience, 100% recommended! The atmosphere and puzzles are out of this world. We\'ll be back soon.', author: 'Carla M.', role: 'Guest' },
...
const Location: React.FC = () => {
  const { t } = useLang();
  const loc = t.temuco.location;
  const [ref, vis] = useInView();
  return (
    <section ref={ref} id="contacto" className="py-28" style={{ background: '#0a0a0a' }}>
      <div className="max-w-6xl mx-auto px-6">
        <div className={`transition-all duration-1000 ease-out ${vis ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <SecHeader label={loc.label} title={<>{loc.title.split(' ').slice(0, -1).join(' ')} <span style={{ color: N }}>{loc.title.split(' ').slice(-1)[0]}</span></>} />
        </div>
    ...
    <div className="max-w-6xl mx-auto px-6">
      <div className="grid md:grid-cols-4 gap-10 mb-10">
        ...
        <div>
          ...
          <p className="text-white/30 text-xs leading-relaxed">{f.tagline}</p>
        </div>

        <div>
          <p className="font-black text-white text-xs uppercase tracking-widest mb-4">{t.temuco.nav.rooms}</p>
          <ul className="space-y-2.5">
            {[[t.shared.home, '#inicio'], [fn.rooms, '#salas'], [fn.howItWorks, '#como-funciona'], [fn.pricing, '#precios'], [fn.contact, '#contacto']].map(([label, href]) => (
              <li key={href}>
